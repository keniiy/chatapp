  ...
  static async logout(req, res) {
    try {
      ...
      if (result.statusCode == 404)
        return errorResponse(res, result.statusCode, result.message);
      logger.info(
        `User logged out successfully with email: ${JSON.stringify(result)}`
      );
      return successResponse(res, 200, "User logged out successfully", result);
    } catch (error) {
      logger.error(
        `Error in logging out user: ${JSON.stringify(error.message)}`
      );
      return errorResponse(res, 500, "Oops! Something went wrong");
    }
  }
